  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!session || !session.user || session.user.role !== 'admin') {
        ...
        throw new Error('Access denied. This workspace is reserved for administrator accounts only.');
      }
      ...
      <Card className="w-full max-w-md border border-red-500/10 bg-slate-950/40 shadow-2xl backdrop-blur-md relative overflow-hidden">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-3 rounded-lg bg-red-950/20 border border-red-500/20 flex gap-2.5 text-[11px] text-red-400 leading-normal">
            ...
            <span>This computer system is for authorized users only. Unauthorized login attempts are strictly prohibited and subject to auditing.</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="email" className="text-xs uppercase tracking-widest text-slate-400 font-bold">Admin Email</Label>
              <Input
                id="email"
                name="email"
                type="email"
