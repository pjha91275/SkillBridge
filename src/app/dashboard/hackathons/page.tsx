...
const HACKATHONS: HackathonItem[] = [
  {
    ...
    description: 'The world\'s largest open innovation hackathon. Solve real-world problem statements submitted by central/state ministries and departments.',
  ...
  },
  {
    ...
    organizer: 'Google',
    description: 'Google\'s signature team-based coding competition. Work in groups of 2 to 4 to solve a complex optimization problem designed by Google engineers.',
  ...
  },
  {
    ...
    organizer: 'Microsoft',
    description: 'The premier global student technology startup competition. Build a tech solution leveraging Microsoft Cloud/Azure solving key social problems.',
  ...
  },
  {
    ...
    description: 'National campus hackathon covering entrepreneurial thinking and design cases. Pitch innovative solutions to Tata executives.',
  ...
  },
  {
    ...
    description: 'Create open-source generative AI tools, chatbots, or workflow engines using modern large language models, agents, and vector databases.',
...
const TEAMMATE_POSTS: TeammatePost[] = [
  {
    name: 'Sachin Jha',
    college: 'TCET Mumbai',
...
export default function HackathonsPage() {
  ...
  const handleToggleRegister = async (hackathonId: string) => {
    if (!profile) return;
  ...
  // Compile tags
  const allTags = ['All', ...Array.from(new Set(HACKATHONS.flatMap((h) => h.tags)))];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-white/5 backdrop-blur-xl flex flex-col md:flex-row justify-between md:items-center gap-4">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-xs font-semibold">
              Find upcoming national coding contests, register interest, and find teammates.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 bg-slate-950/80 px-4 py-2.5 rounded-2xl border border-white/5 self-start md:self-auto">
            <div className="text-center px-3 border-r border-white/5">
              <span className="text-[9px] text-slate-500 block font-bold uppercase">Total Listings</span>
              <span className="text-lg font-black text-white">{HACKATHONS.length}</span>
            </div>
            <div className="text-center px-3">
              <span className="text-[9px] text-indigo-400 block font-bold uppercase">My Registrations</span>
              <span className="text-lg font-black text-indigo-400">{registeredIds.length}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 bg-slate-900/20 p-4 rounded-2xl border border-white/5">
          ...
          <div className="flex items-center gap-2 bg-slate-950 border border-white/5 rounded-xl px-3 py-2 text-xs">
            ...
            >
              {allTags.map((tag) => (
                <option key={tag} value={tag} className="bg-slate-900">{tag === 'All' ? 'All Focus Areas' : tag}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-center gap-2 bg-slate-950 border border-white/5 rounded-xl px-3 py-2 text-xs">
            ...
            <select
              ...
              className="bg-transparent text-slate-300 outline-none w-full border-none cursor-pointer font-medium"
            >
              <option value="All" className="bg-slate-900">All Timelines</option>
              <option value="Open" className="bg-slate-900">Registration Open</option>
              <option value="Upcoming" className="bg-slate-900">Upcoming Contests</option>
              <option value="Registered" className="bg-slate-900">My Registered Hackathons</option>
            </select>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            {filteredHackathons.length > 0 ? (
              filteredHackathons.map((h) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute top-0 right-0 h-1.5 w-24 rounded-bl-lg ${
                    ...
                    <div className="space-y-3 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {h.tags.map((t) => (
                          <span key={t} className="text-[9px] font-bold px-2 py-0.5 rounded bg-slate-950 text-slate-500 border border-white/5">
                            {t}
                          </span>
                        ))}
                      </div>
              ...
              <div className="bg-slate-900/10 border border-dashed border-white/5 p-12 text-center rounded-2xl">
                ...
                <p className="text-slate-500 text-xs italic">No hackathons match the selected filters.</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/40 border border-white/5 p-5 rounded-2xl space-y-4 backdrop-blur-xl">
              ...
              <p className="text-[10px] text-slate-500 leading-relaxed">
                Connect with classmates looking to form teams and register for upcoming coding events.
              </p>

              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                {TEAMMATE_POSTS.map((post, idx) => (
                  <div key={idx} className="bg-slate-950/70 border border-white/5 p-4.5 rounded-xl space-y-2">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <span className="text-xs font-bold text-white block">{post.name}</span>
                        <span className="text-[8px] text-slate-500 font-semibold block">{post.college} • {post.branch}</span>
                      </div>
                      <span className="text-[8px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-400 font-extrabold truncate max-w-28 uppercase">
                    ...
                    <div className="flex flex-wrap gap-1 pt-1.5 border-t border-white/2">
                      {post.needs.map((n) => (
                        <span key={n} className="text-[8px] px-2 py-0.5 rounded bg-slate-900 text-slate-400 border border-white/5 font-extrabold uppercase">
                          {n}
                        </span>
                      ))}
                    </div>
              ...
              <div className="pt-2">
                <Button className="w-full bg-slate-950 text-indigo-400 border border-indigo-500/10 hover:bg-slate-900 text-xs font-bold py-2 px-4 rounded-xl cursor-pointer flex items-center justify-center gap-1">
            ...
            <div className="bg-indigo-950/10 border border-indigo-500/5 p-5 rounded-2xl space-y-2">
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed">
                Hackathons contribute directly to your **Placement Readiness Score**. Solving problem statements yields points under "Technical Projects" and highlights your ability to build functional code prototypes.
              </p>
            </div>
