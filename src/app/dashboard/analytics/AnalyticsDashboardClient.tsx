...
interface AnalyticsDashboardClientProps {
  data: {
    ...
    skillCompletionPercent: number;
    dsaCompletionPercent: number;
...
export default function AnalyticsDashboardClient({ data }: AnalyticsDashboardClientProps) {
  ...
  const dsaTopicData = [
    { subject: 'Arrays & Strings', Solved: Math.min(100, Math.round(dsaBase * 1.1)) },
    { subject: 'Linked Lists', Solved: Math.min(100, Math.round(dsaBase * 0.9)) },
    { subject: 'Stacks & Queues', Solved: Math.min(100, Math.round(dsaBase * 0.8)) },
    { subject: 'Trees & Graphs', Solved: Math.min(100, Math.round(dsaBase * 0.5)) },
    { subject: 'Dynamic Prog.', Solved: Math.min(100, Math.round(dsaBase * 0.3)) },
    { subject: 'DBMS Queries', Solved: Math.min(100, Math.round(dsaBase * 0.7)) },
  ];
  ...
  const defaultSkillsData = [
    { skill: 'Languages', Status: profile.skills.length > 0 ? 80 : 20, color: '#8b5cf6' },
    { skill: 'Frameworks', Status: profile.skills.length > 2 ? 60 : 20, color: '#8b5cf6' },
    { skill: 'Databases', Status: profile.skills.length > 4 ? 70 : 20, color: '#8b5cf6' },
    { skill: 'Dev Tools', Status: profile.skills.length > 5 ? 50 : 20, color: '#8b5cf6' },
  ];

  const actualSkillsData = skillsBreakdownData.length > 0 ? skillsBreakdownData : defaultSkillsData;

  // 4. Chart: Goal Completion Trend
  const goalTrendData = [
    { name: 'Pending', value: goalsSummary.pending || 0, color: '#64748b' },
    { name: 'In Progress', value: goalsSummary.inProgress || 0, color: '#f59e0b' },
  ...
  if (profile.targetRole) {
    insights.push(`You have completed ${roadmapProgressPercent}% of your ${profile.targetRole} Roadmap.`);
  } else {
    insights.push("Set a target career role in settings/profile to map your personalized carrier roadmap progress.");
  }

  if (missingSkills.length > 0) {
    insights.push(`${missingSkills.slice(0, 2).join(' and ')} are currently your biggest technical skill gaps.`);
  } else if (profile.targetRole) {
    insights.push(`Excellent! You have acquired all core technical requirements mapped for ${profile.targetRole}.`);
  }

  const projectsCount = profile.projects?.length || 0;
  if (projectsCount < 2) {
    const pointsIncrease = (2 - projectsCount) * 10;
    insights.push(`Adding ${2 - projectsCount} more project${2 - projectsCount > 1 ? 's' : ''} to your profile can increase your Readiness score by ${pointsIncrease}%.`);
  } else {
    insights.push("You meet the recommended project standards (2+ projects) for recruitment filters.");
  }

  // Circular progress ring generator
  const renderProgressRing = (percentage: number, label: string, colorClass: string) => {
    const radius = 36;
    const strokeWidth = 5;
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-slate-900/40 border border-white/5 relative overflow-hidden">
        ...
        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-2">{label}</span>
      </div>
    );
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Visual insights, trend graphics, and benchmarks mapping your campus placement readiness.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 glass-panel p-6 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">Placement recruitment filter probability score.</p>
          </div>
          ...
          <div className="my-6 flex items-baseline gap-2">
            ...
            <span className="text-slate-500 font-bold text-sm uppercase">/ 100 pts</span>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-semibold">
              ...
              <span className="text-indigo-400">
                {readinessScore >= 80 ? 'Excellent' : readinessScore >= 60 ? 'Good' : 'Needs Work'}
              </span>
            </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {renderProgressRing(skillCompletionPercent, 'Skills Complete', 'text-violet-500')}
          {renderProgressRing(dsaCompletionPercent, 'DSA Progress', 'text-indigo-500')}
          {renderProgressRing(roadmapProgressPercent, 'Roadmap Steps', 'text-blue-500')}
          {renderProgressRing(resumeCompletionPercent, 'Resume Score', 'text-emerald-500')}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="glass-panel p-6 space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Readiness improvements calculated over the past 6 weeks.</p>
          </div>
        ...
        <Card className="glass-panel p-6 space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Estimated questionnaire completion percentages per DSA topic.</p>
          </div>
          <div className="h-64 w-full text-xs font-semibold">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={dsaTopicData}>
        ...
        <Card className="glass-panel p-6 space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Status index comparison of required profile competencies.</p>
          </div>
          <div className="h-64 w-full text-xs font-semibold">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={actualSkillsData} layout="vertical">
                ...
                <Tooltip
                  ...
                  formatter={(val) => [`${val === 100 ? 'Acquired' : 'Pending Gap'}`, 'Status']}
                />
        ...
        <Card className="glass-panel p-6 space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Distribution counts of active placement target goals in Mapped state.</p>
          </div>
          <div className="h-64 w-full text-xs font-semibold">
            {goalsSummary.total > 0 ? (
              ...
              <div className="h-full w-full flex flex-col items-center justify-center text-center space-y-2">
                ...
                <p className="text-xs text-slate-400">No goals created yet. Visit your Goals Tracker to seed lists.</p>
                <Link href="/dashboard/goals">
                  <Button variant="outline" size="sm" className="text-[10px] mt-1 border-white/5">
                    Create Goal
                  </Button>
                </Link>
              </div>
