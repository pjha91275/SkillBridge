...
const CATEGORIES = ['DSA', 'Aptitude', 'DBMS', 'Operating Systems', 'Computer Networks', 'OOP', 'Web Development', 'Interview Preparation', 'Other'];
...
export default function GoalsPage() {
  ...
  // Open edit modal
  const handleEditOpen = (item: GoalItem) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        editingId ? 'Weekly goal updated successfully!' : 'Weekly goal created successfully!',
        'success'
      );
  ...
  const handleUpdateStatus = async (item: GoalItem, nextStatus: 'Pending' | 'In Progress' | 'Completed') => {
    try {
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Structure, manage, and complete your weekly placement study milestones.
            </p>
          </div>
        ...
        <Card className="glass-panel p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 items-center">
            ...
            <div className="space-y-1 text-center md:text-left">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total goals</span>
            ...
            <div className="space-y-1 text-center md:text-left border-l border-white/5 pl-0 md:pl-6">
              ...
              <h3 className="text-3xl font-black text-white">{completedCount}</h3>
            </div>

            {/* In Progress & Pending */}
            <div className="space-y-1 text-center md:text-left border-l border-white/5 pl-0 md:pl-6">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Remaining / In progress</span>
              <h3 className="text-3xl font-black text-white">{pendingCount + inProgressCount}</h3>
            </div>
            ...
            <div className="space-y-2 col-span-2 md:col-span-1 border-t md:border-t-0 md:border-l border-white/5 pt-4 md:pt-0 pl-0 md:pl-6">
              <div className="flex justify-between items-center text-[10px] font-bold text-slate-400">
                <span className="uppercase tracking-wider">Completion Index</span>
        ...
        {isLoading ? (
          ...
          <Card className="glass-panel p-12 text-center space-y-4 max-w-md mx-auto">
            ...
            <h3 className="text-lg font-bold text-white">Wipe the slate clean</h3>
            <p className="text-slate-400 text-sm leading-relaxed">
              No weekly study goals configured yet. Set milestones for data structures, aptitude, and databases to lock in placements.
            </p>
            <Button
              onClick={handleAddOpen}
            ...
            <div className="space-y-4 bg-slate-900/10 p-4 rounded-2xl border border-white/5 backdrop-blur-xs flex flex-col min-h-[450px]">
              <div className="flex justify-between items-center pb-2 border-b border-white/5">
                <span className="text-xs font-extrabold uppercase tracking-wider text-slate-400 flex items-center gap-2">
                  <Circle className="h-3.5 w-3.5 text-slate-500" /> Pending ({goals.filter((g) => g.status === 'Pending').length})
                </span>
              </div>
            ...
            <div className="space-y-4 bg-slate-900/10 p-4 rounded-2xl border border-white/5 backdrop-blur-xs flex flex-col min-h-[450px]">
              <div className="flex justify-between items-center pb-2 border-b border-white/5">
                <span className="text-xs font-extrabold uppercase tracking-wider text-amber-400 flex items-center gap-2">
                  <Clock className="h-3.5 w-3.5 text-amber-500 shrink-0" /> In Progress ({goals.filter((g) => g.status === 'In Progress').length})
                </span>
              </div>
              <div className="space-y-4 flex-1 overflow-y-auto pr-1">
                {goals
            ...
            <div className="space-y-4 bg-slate-900/10 p-4 rounded-2xl border border-white/5 backdrop-blur-xs flex flex-col min-h-[450px]">
              <div className="flex justify-between items-center pb-2 border-b border-white/5">
                <span className="text-xs font-extrabold uppercase tracking-wider text-emerald-400 flex items-center gap-2">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> Completed ({goals.filter((g) => g.status === 'Completed').length})
                </span>
              </div>
              <div className="space-y-4 flex-1 overflow-y-auto pr-1">
                {goals
                ...
                <tbody>
                  {goals.map((item) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 text-xs font-medium text-indigo-300">{item.category}</td>
                      ...
                      <td className="py-4 px-4">
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${getPriorityClass(item.priority)}`}>
                          {item.priority}
                        </span>
                      </td>
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-1.5 text-xs font-semibold">
                          ...
                          <span className={item.status === 'Completed' ? 'text-emerald-400' : item.status === 'In Progress' ? 'text-amber-400' : 'text-slate-400'}>
                            {item.status}
                          </span>
                        </div>
                      </td>
          ...
          >
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <Label htmlFor="formTitle" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Goal Title</Label>
                <Input
                  id="formTitle"
                  value={formTitle}
              ...
              <div className="space-y-1">
                <Label htmlFor="formDesc" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Description (optional)</Label>
                <textarea
                  id="formDesc"
                  value={formDesc}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <Label htmlFor="formCategory" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Category</Label>
                  <select
                    id="formCategory"
                    value={formCategory}
                ...
                <div className="space-y-1">
                  <Label htmlFor="formDeadline" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Deadline Date</Label>
                  <Input
                    id="formDeadline"
                    type="date"
                    value={formDeadline}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <Label htmlFor="formPriority" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Priority Level</Label>
                  <select
                    id="formPriority"
                    value={formPriority}
                ...
                <div className="space-y-1">
                  <Label htmlFor="formStatus" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Status</Label>
                  <select
                    id="formStatus"
                    value={formStatus}
...
function GoalCard({
  ...
  onUpdateStatus: (item: GoalItem, nextStatus: any) => void;
    ...
    <Card className="glass-panel p-4 flex flex-col justify-between hover:border-indigo-500/10 group/card transition-all duration-300 min-h-[140px]">
      <div className="space-y-2">
        <div className="flex justify-between items-start gap-2">
          <span className="text-[9px] font-extrabold text-indigo-300 uppercase tracking-wider">{item.category}</span>
          <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full ${getPriorityClass(item.priority)}`}>
            {item.priority}
          </span>
        </div>
        <div>
      ...
      <div className="pt-3 border-t border-white/5 flex items-center justify-between mt-3 text-[10px] text-slate-500 font-medium">
        ...
        <div className="flex items-center gap-1.5 opacity-80 md:opacity-0 group-hover/card:opacity-100 transition-opacity">
          {item.status !== 'Completed' && (
            <Button
              onClick={() => onUpdateStatus(item, item.status === 'Pending' ? 'In Progress' : 'Completed')}
              variant="ghost"
              size="sm"
