...
interface ResourceItem {
  ...
  description: string;
  category: string;
  resourceType: 'PDF' | 'Video' | 'Website' | 'Notes';
  resourceLink: string;
...
export default function AdminResourcesPage() {
  ...
  const handleEditOpen = (item: ResourceItem) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        editingId ? 'Resource updated successfully!' : 'Resource added successfully!',
        'success'
      );
    ...
    <div className="flex-1 bg-slate-950 p-6 md:p-12 space-y-8 animate-slide-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Configure, update, or remove placement preparation resources in the hub database.
          </p>
        </div>
        <Button
      ...
      {isLoading ? (
        ...
        <Card className="glass-panel p-6 space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-12 w-full bg-slate-900/60 rounded-xl skeleton-pulse" />
          ))}
        </Card>
              ...
              <tbody>
                {filteredResources.map((item) => (
                  ...
                  >
                    ...
                    {/* Category */}
                    <td className="py-4 px-4 text-indigo-300">{item.category}</td>
                    ...
                    <td className="py-4 px-4">
                      <span className={`text-[8px] font-black px-2 py-0.5 rounded-full ${getDifficultyBadge(item.difficultyLevel)}`}>
        ...
        <Card className="glass-panel p-12 text-center space-y-4 max-w-md mx-auto">
          ...
          <p className="text-slate-400 text-sm">
            We couldn't find any database resources matching your category or search queries.
          </p>
        </Card>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          ...
          >
            <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4 border-b border-border/5 pb-2">
              {editingId ? <Edit className="h-5 w-5 text-indigo-400" /> : <Plus className="h-5 w-5 text-indigo-400" />}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <Label htmlFor="formTitle" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Title</Label>
                <Input
                  id="formTitle"
                  value={formTitle}
              ...
              <div className="space-y-1">
                <Label htmlFor="formDesc" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Description</Label>
                <textarea
                  id="formDesc"
                  value={formDesc}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <Label htmlFor="formType" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Type</Label>
                  <select
                    id="formType"
                    value={formType}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1 col-span-1">
                  <Label htmlFor="formDifficulty" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">Difficulty</Label>
                  <select
                    id="formDifficulty"
                ...
                <div className="space-y-1 col-span-1">
                  <Label htmlFor="formLink" className="text-xs text-slate-300 font-semibold uppercase tracking-wider">URL Link</Label>
                  <Input
                    id="formLink"
                    type="url"
                    value={formLink}
