...
export default async function AdminDashboardPage() {
  ...
  const maxCount = categoryCounts.length > 0 ? Math.max(...categoryCounts.map((c) => c.count)) : 1;
  ...
  allProfiles.forEach((profile) => {
    ...
    const score = skillsPoints + projectsPoints + dsaPoints + certsPoints + profilePoints;
    totalScoreSum += score;
  });

  const averageReadiness = studentCount > 0 ? Math.round(totalScoreSum / studentCount) : 0;
  const averageCgpa = cgpaCount > 0 ? (totalCgpaSum / cgpaCount).toFixed(2) : '0.00';
    ...
    <div className="flex-1 bg-slate-950 p-6 md:p-12 space-y-8 animate-slide-up">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">
          Review system metrics, student activity indexes, and preparation benchmarks.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="glass-panel p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total Resources</p>
          </div>
        </Card>
        ...
        <Card className="glass-panel p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total Goals</p>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="glass-panel p-6 space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Resource distribution counts index across all categories.</p>
          </div>
                ...
                return (
                  <div key={cat._id} className="space-y-1">
                    <div className="flex justify-between text-xs font-semibold">
                      ...
                      <span className="text-indigo-400">{cat.count} resource{cat.count > 1 ? 's' : ''}</span>
                    </div>
        ...
        <Card className="glass-panel p-6 space-y-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Aggregated preparation activity calculated from active profiles.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-2">
            ...
            <div className="p-4 rounded-xl bg-slate-950/60 border border-white/5 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Avg Readiness</p>
                <p className="text-xl font-black text-white">{averageReadiness}%</p>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/60 border border-white/5 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Avg CGPA</p>
                <p className="text-xl font-black text-white">{averageCgpa} / 10</p>
              </div>
            </div>
          ...
          <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/10 text-xs text-slate-400 leading-relaxed">
            <span className="font-bold text-indigo-400 block mb-0.5">Scouter Visibility Info</span>
            Placement readiness score evaluates all parameters (CGPA, skills, projects, certifications, and goals completion). Regular updates to profile databases directly enhance these averages.
          </div>
        </Card>
