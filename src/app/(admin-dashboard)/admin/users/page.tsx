...
export default function AdminUsersPage() {
  ...
  const handleDeleteUser = async (user: UserItem) => {
    if (!confirm(`Are you sure you want to permanently delete user account "${user.name}"? This will wipe all their profiles, goals, and checklists from the database.`)) {
      return;
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Browse registered peer directories and perform administrator account deletions.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
            ...
            <table className="w-full text-left text-sm text-slate-300">
              ...
              <tbody>
                {filteredUsers.map((item) => (
                  ...
                  >
                    {/* Name */}
                    <td className="py-4 px-4 font-extrabold text-white">{item.name}</td>
                    ...
                    <td className="py-4 px-4">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 mt-0.5">{item.branch}</p>
                      </div>
                    </td>
        ...
        <Card className="glass-panel p-12 text-center space-y-4 max-w-md mx-auto">
          ...
          <p className="text-slate-400 text-sm">
            We couldn't find any registered student accounts matching your current search parameters.
          </p>
        </Card>
      )}
